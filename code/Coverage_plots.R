...
# Load Coverages ----------------------------------------------------------
load(file = here("results/Simulations/Coverage/RegNormal_CIs.RData"))
coverage_wermuth_array_RegNormal <- decision_wermuth_array
load(file = here("results/Simulations/Coverage/RegCauchy_CIs.RData"))
coverage_wermuth_array_RegCauchy <- decision_wermuth_array
load(file = here("results/Simulations/Coverage/MultNorm_CIs.RData"))
coverage_wermuth_array_MultNormal <- decision_wermuth_array
load(file = here("results/Simulations/Coverage/MultCauchy_CIs.RData"))
coverage_wermuth_array_MultCauchy <- decision_wermuth_array
load(file = here("results/Simulations/Coverage/3x3SU_CIs.RData"))
coverage_wermuth_array_3x3_SU <- decision_wermuth_array
load(file = here("results/Simulations/Coverage/3x3UU_CIs.RData"))
...
ggsave(filename = here("results/Plots/Coverage.pdf"), device = cairo_pdf, width = 15, height = 30, units = "cm")

fin_plot_wide <- ggplot2::ggplot(data = cov_wermuth_array) +
  geom_line(mapping = aes(y = Coverage, x = gamma), color = "red") +
  geom_hline(yintercept=c(0.9), col="black") +
...
ggsave(filename = here("results/Plots/Coverage_wide.pdf"), device = cairo_pdf, width = 30, height = 15, units = "cm")
